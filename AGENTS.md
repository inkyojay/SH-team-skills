...
Codex 개인 스킬은 `~/.Codex/skills/`에서 로드됩니다. 팀 저장소 스킬을 Codex에서 바로 쓰려면 원본 폴더를 심볼릭 링크로 연결합니다.

```bash
ln -sfn /Users/inkyo/Projects/team-skills/skills/promotion/event-page-campaign \
  ~/.Codex/skills/event-page-campaign
```
...
| Codex API | `ANTHROPIC_API_KEY` | viral-shorts-maker, content-pipeline |
...
- **MCP 서버**: `media-dashboard` (Codex에 글로벌 등록됨)
...
3. Codex에 MCP 서버 등록:
   ```bash
   Codex mcp add --scope user media-dashboard -- npx --prefix /path/to/media-dashboard/mcp-server tsx /path/to/media-dashboard/mcp-server/index.ts
   ```
4. Codex 재시작 후 사용 가능
