# 썬데이허그 마케팅 스킬팩

마케팅 업무 자동화를 위한 **42개 스킬 + 13개 에이전트** 패키지입니다.

> 전체 스킬 목록은 [SKILL-CATALOG.md](SKILL-CATALOG.md) 참조

---

## 폴더 구조

```
team-skills/
├── skills/                     # 모든 스킬 (42개)
│   ├── content-creation/       # 콘텐츠 제작 (2)
│   │   ├── page-builder/       # 상세페이지 빌더
│   │   └── card-news/          # 카드뉴스
│   ├── video/                  # 영상 제작 (2)
│   │   ├── reels-editor/       # 릴스 편집
│   │   └── remotion/           # React 영상
│   ├── advertising/            # 광고 (1)
│   │   └── meta-ads/           # 메타 광고
│   ├── brand/                  # 브랜드 (5)
│   │   ├── brand-dna/          # 브랜드 DNA 추출
│   │   ├── product-analyzer/   # 제품 분석
│   │   └── ...
│   ├── marketing/              # 마케팅 전략 (22)
│   │   ├── copywriting/        # 카피라이팅
│   │   ├── paid-ads/           # 유료 광고
│   │   ├── page-cro/           # 전환율 최적화
│   │   └── ...
│   └── tools/                  # 유틸리티 (9)
│       ├── skill-creator/      # 스킬 생성
│       └── ...
├── agents/                     # AI 에이전트 (13개)
└── commands/                   # 슬래시 커맨드
```

---

## 사용 방법

**슬래시 명령어가 아닙니다.** 자연어로 요청하세요.

```
"상세페이지 만들어줘"
"카드뉴스 만들어줘"
"메타 광고 기획해줘"
"랜딩페이지 CRO 분석해줘"
```

---

## 주요 스킬

### 콘텐츠 제작

| 스킬 | 위치 | 기능 |
|------|------|------|
| **상세페이지 빌더** | `skills/content-creation/page-builder/` | 이커머스 상품 상세페이지 |
| **카드뉴스** | `skills/content-creation/card-news/` | 인스타그램 카드뉴스 |
| **릴스 편집** | `skills/video/reels-editor/` | 9:16 릴스 영상 |
| **Remotion** | `skills/video/remotion/` | React 영상 제작 |
| **메타 광고** | `skills/advertising/meta-ads/` | Facebook/Instagram 광고 |

### 브랜드 도구

| 스킬 | 위치 | 기능 |
|------|------|------|
| **브랜드 DNA** | `skills/brand/brand-dna/` | 웹사이트 브랜드 분석 |
| **제품 분석** | `skills/brand/product-analyzer/` | 상품 페이지 분석 |

### 마케팅 전략 (22개)

| 카테고리 | 스킬 |
|----------|------|
| **광고** | paid-ads, page-cro, ab-test |
| **콘텐츠** | copywriting, social-content, email-sequence, video-script |
| **분석** | competitor-analysis, csv-analyzer, analytics-tracking |
| **전략** | launch-strategy, pricing-strategy, marketing-psychology |
| **디자인** | canvas-design, social-media-designer |

### 이미지 도구 (2개)

| 스킬 | 기능 |
|------|------|
| **batch-image-transform** | 상품 이미지 배경 교체/장면 합성 (배치) |
| **tone-match-local** | 레퍼런스 톤/색감을 상품 이미지에 일괄 적용 |

### 유틸리티 (9개)

| 스킬 | 기능 |
|------|------|
| skill-creator | 새 스킬 생성 |
| subagent-creator | 서브에이전트 생성 |
| youtube-collector | YouTube 콘텐츠 수집 |
| html2img | HTML → 이미지 변환 |

---

## 에이전트 (13개)

| 에이전트 | 역할 |
|----------|------|
| **마케팅 허브** | "마케팅 도와줘" | 마케팅 업무 총괄, 워크플로우 설계 |
| **경쟁사 분석** | "코니바이에린 분석해줘" | 경쟁 브랜드 SWOT 분석 및 대응 전략 |
| **시장 조사** | "시장 조사해줘" | 시장 규모, 트렌드, 소비자 인사이트 |
| **콘텐츠 검토** | "콘텐츠 검토해줘" | 브랜드 일관성, 품질 검토 |
| **데이터 리포트** | "리포트 분석해줘" | 광고 성과 데이터 분석 |
| **메타 광고 에이전트** | "메타 광고 전략 세워줘" | 대화형 Meta 광고 캠페인 기획 |

### 브랜드 관리 에이전트 (3개)

| 에이전트 | 요청 예시 | 역할 |
|----------|----------|------|
| **브랜드 설정 위자드** | "새 브랜드 설정해줘" | 신규 브랜드 초기 설정 가이드 |
| **브랜드 업데이터** | "브랜드 정보 업데이트해줘" | 기존 브랜드 정보 수정 |
| **브랜드 로고 파인더** | "로고 찾아줘" | 브랜드 로고 검색 및 추출 |

### 스킬 생성 도구 (4개)

| 도구 | 요청 예시 | 기능 |
|------|----------|------|
| **스킬 생성기** | "새 스킬 만들어줘" | 새로운 스킬 템플릿 생성 |
| **서브에이전트 생성** | "에이전트 만들어줘" | 서브에이전트 정의 파일 생성 |
| **슬래시 명령어 생성** | "명령어 만들어줘" | 슬래시 명령어 스킬 생성 |
| **훅 생성기** | "훅 만들어줘" | 이벤트 훅 생성 |

---

## 빠른 시작 예시

| 하고 싶은 것 | 이렇게 요청하세요 |
|-------------|------------------|
| 상세페이지 제작 | "아기띠 상세페이지 만들어줘" |
| 카드뉴스 제작 | "이 유튜브 영상으로 카드뉴스 만들어줘" |
| 릴스 제작 | "이 영상 릴스로 편집해줘" |
| 광고 기획 | "슬리핑백 인스타 광고 기획해줘" |
| 경쟁사 분석 | "스와들업 분석해줘" |
| 시장 조사 | "신생아 수면용품 시장 조사해줘" |
| 톤 매치 | "상품사진 톤 매치 해줘. 레퍼런스 ref.jpg" |
| 배경 교체 | "상품사진 카페 배경으로 변환해줘" |
| CSS 변환 | "page.html 인라인으로 변환해줘" |
| 브랜드 분석 | "이 사이트 브랜드 DNA 추출해줘" |
| 콘텐츠 검토 | "방금 만든 상세페이지 검토해줘" |
| 리포트 분석 | "이 광고 성과 데이터 분석해줘" |

---

## 스킬별 상세 정보

### 1. 상세페이지 빌더
**폴더:** `page-builder/`

**요청 예시:**
- "상세페이지 만들어줘"
- "OO 제품 페이지 작성해줘"
- "이 URL 분석해서 상세페이지 리뉴얼해줘"
- "썬데이허그 스타일로 상세페이지 만들어줘"

**기능:**
- 이커머스 상품 상세페이지 HTML 생성
- 브랜드별 컬러 시스템 적용
- 기승전결 스토리텔링 구조
- 31종 섹션 템플릿 제공

---

### 2. 카드뉴스 제작
**폴더:** `card-news/`

**요청 예시:**
- "카드뉴스 만들어줘"
- "이 유튜브 영상으로 카드뉴스 만들어줘"
- "이 블로그 글 카드뉴스로 변환해줘"

**기능:**
- YouTube, 웹페이지, PDF에서 콘텐츠 추출
- 8-10장 카드 구조 자동 설계
- 나노바나 API로 배경 이미지 생성
- Sunday Hug 스타일 템플릿

---

### 3. 릴스 영상 편집
**폴더:** `reels-editor/`

**요청 예시:**
- "이 영상 릴스로 만들어줘"
- "영상 세로로 변환하고 자막 넣어줘"
- "10초~40초 구간만 잘라서 릴스로"

**기능:**
- Instagram Reels 규격 (9:16, 1080x1920) 변환
- AI 영상 분석 (Gemini Vision)
- 블러 배경 세로 변환
- 텍스트 오버레이, CTA 버튼 추가

**필수 설치:** `ffmpeg`, `GEMINI_API_KEY`

---

### 4. 메타 광고 기획
**폴더:** `meta-ads/`

**요청 예시:**
- "메타 광고 기획해줘"
- "인스타 광고 캠페인 만들어줘"
- "프로모션 광고 전략 세워줘"

**기능:**
- 대화형 광고 캠페인 전략 수립
- 소구점 발굴 및 컨셉 제안
- 타겟 오디언스 설계
- 크리에이티브 가이드 제작

---

### 5. 브랜드 DNA 추출
**폴더:** `brand-tools/brand-dna/`

**요청 예시:**
- "브랜드 분석해줘"
- "이 사이트 DNA 추출해줘"
- "무드보드 만들어줘"

**분석 항목:**
- 컬러 팔레트
- 타이포그래피
- 브랜드 퍼스낼리티
- 톤앤매너

---

### 6. 톤 매치 (이미지 톤 보정)
**폴더:** `skills/tone-match-local/`

**요청 예시:**
- "상품사진 톤 매치 해줘"
- "이 폴더 이미지들을 ref.jpg 색감으로 맞춰줘"
- "톤앤매너 변환해줘, 강도 50%"

**기능:**
- 레퍼런스 이미지의 색감/톤/분위기를 상품 이미지에 일괄 적용
- 폴더 단위 배치 처리 (매수 제한 없음)
- 강도 조절 (0-100%, 기본 70%)
- Gemini AI 모델 선택 (Flash/Pro)

**필수 설치:** `GOOGLE_AI_API_KEY`, `cd scripts && npm install`

---

### 7. 배치 이미지 변환 (배경 교체)
**폴더:** `skills/batch-image-transform/`

**요청 예시:**
- "상품사진 배경 바꿔줘"
- "이 폴더 이미지를 카페 배경으로 변환해줘"
- "reference.jpg 스타일로 변환해줘"

**기능:**
- 레퍼런스/HTML/프롬프트 기반 배경 교체
- 폴더 단위 배치 처리
- 비율 선택 (1:1, 9:16, 16:9)

**필수 설치:** `GOOGLE_AI_API_KEY`, `cd scripts && npm install`

---

### 8. 제품 분석
**폴더:** `brand-tools/product-analyzer/`

**요청 예시:**
- "제품 분석해줘"
- "이 URL 제품 정보 정리해줘"

**기능:**
- 제품 URL에서 정보 수집
- 특징, 스펙, 가격 분석
- 분석 리포트 생성

---

## 브랜드 컨텍스트

이 스킬팩은 **썬데이허그** 브랜드에 최적화되어 있습니다.

| 항목 | 내용 |
|------|------|
| 브랜드 | 썬데이허그 (SUNDAYHUG) |
| 카테고리 | 프리미엄 베이비 슬립케어 |
| 타겟 | 0-36개월 아기 부모 (30대 여성) |
| 톤앤매너 | 따뜻하고 신뢰감 있는 육아 전문가 |
| 핵심 가치 | 따뜻함, 편안함, 자연친화, 신뢰 |

### 브랜드 컬러

| 이름 | HEX | 용도 |
|------|-----|------|
| 다크 브라운 | #A38068 | 메인 포인트 |
| 데일리 크림 | #FFFBF5 | 밝은 배경 |
| 오트 | #EAE2D5 | 중간 배경 |
| 오렌지 | #ff8605 | CTA, 강조 |
| 코랄 | #eaccca | 서브 배경 |

### 주요 경쟁사

코니바이에린, 스와들업, 에이든아네이, 도노도노

---

## 폴더 구조

```
skills/
├── SKILL.md                 # 이 문서
│
├── agents/                  # 에이전트 정의 파일 (10개)
│   ├── sundayhug-marketing-hub.md
│   ├── competitor-analyzer.md
│   ├── market-researcher.md
│   ├── content-quality-reviewer.md
│   ├── data-report-analyzer.md
│   ├── brand-setup-wizard.md
│   ├── brand-updater.md
│   ├── brand-logo-finder.md
│   ├── reels-editor-agent.md
│   └── meta-ads-agent.md
│
├── page-builder/            # 상세페이지 빌더
├── card-news/               # 카드뉴스 제작
├── reels-editor/            # 릴스 영상 편집
├── meta-ads/                # 메타 광고 기획
│
├── brand-tools/             # 브랜드 도구
│   ├── brand-dna/           # 브랜드 DNA 추출
│   └── product-analyzer/    # 제품 분석
│
├── commands/                # 유틸리티 명령어
│   ├── inline-css.md        # CSS 인라인 변환
│   └── capture-sections.md  # HTML 섹션 캡처
│
├── cc-skills/               # 스킬 생성 도구
│   ├── skill-creator/
│   ├── subagent-creator/
│   ├── slash-command-creator/
│   ├── hook-creator/
│   ├── html-section-capture/
│   └── youtube-collector/
│
├── external-skills/         # 외부 마케팅 스킬 (17개)
│   ├── page-cro/            # 랜딩페이지 CRO 분석
│   ├── copywriting/         # 마케팅 카피라이팅
│   ├── seo-audit/           # SEO 감사
│   ├── email-sequence/      # 이메일 시퀀스
│   ├── csv-analyzer/        # 데이터 분석
│   ├── competitor-analysis/ # 경쟁사 분석
│   ├── ab-test/             # A/B 테스트
│   ├── marketing-psychology/# 마케팅 심리학 (70+기법)
│   ├── social-content/      # 소셜미디어 콘텐츠
│   ├── pricing-strategy/    # 가격 전략
│   ├── launch-strategy/     # 출시 전략
│   ├── paid-ads/            # 유료 광고 캠페인
│   ├── referral-program/    # 추천 프로그램
│   ├── analytics-tracking/  # 분석 추적 (GA4/Pixel)
│   ├── video-script/        # 영상 스크립트
│   ├── review-management/   # 리뷰 관리
│   └── marketing-ideas/     # 140개 마케팅 아이디어
│
└── upload-package/          # 배포용 패키지 (zip 파일들)
```

---

## 총 스킬 수: 39개

- 콘텐츠 제작: 4개
- 브랜드 도구: 2개
- 유틸리티: 3개
- 마케팅 에이전트: 6개
- 브랜드 관리 에이전트: 3개
- 스킬 생성 도구: 4개
- **외부 마케팅 스킬: 17개** (신규)

---

## 외부 마케팅 스킬 (신규)

Corey Haines 마케팅 스킬을 기반으로 썬데이허그에 맞게 커스터마이징한 스킬입니다.

### 1. 랜딩페이지 CRO
**폴더:** `external-skills/page-cro/`

**요청 예시:**
- "페이지 전환율 분석해줘"
- "랜딩페이지 개선점 찾아줘"

**분석 항목:**
- 첫인상 (Above the Fold)
- 가치 제안
- 신뢰 요소
- CTA 최적화
- 모바일 최적화

---

### 2. 카피라이팅
**폴더:** `external-skills/copywriting/`

**요청 예시:**
- "광고 카피 작성해줘"
- "헤드라인 5개 만들어줘"

**프레임워크:**
- AIDA (주목-흥미-욕망-행동)
- PAS (문제-자극-해결)
- 4U 헤드라인

---

### 3. SEO 감사
**폴더:** `external-skills/seo-audit/`

**요청 예시:**
- "SEO 분석해줘"
- "검색 최적화 상태 확인해줘"

**분석 항목:**
- 기술적 SEO
- 온페이지 SEO
- 키워드 리서치
- 구조화된 데이터

---

### 4. 이메일 시퀀스
**폴더:** `external-skills/email-sequence/`

**요청 예시:**
- "웰컴 이메일 시퀀스 만들어줘"
- "장바구니 이탈 이메일 작성해줘"

**시퀀스 유형:**
- 웰컴 시퀀스
- 장바구니 이탈
- 구매 후 팔로업
- 재활성화

---

### 5. CSV 데이터 분석
**폴더:** `external-skills/csv-analyzer/`

**요청 예시:**
- "매출 데이터 분석해줘"
- "고객 데이터 인사이트 추출해줘"

**분석 유형:**
- 매출 분석
- 고객 세그먼트
- 마케팅 성과

---

### 6. 경쟁사 분석
**폴더:** `external-skills/competitor-analysis/`

**요청 예시:**
- "코니바이에린 분석해줘"
- "경쟁사 벤치마킹해줘"

**분석 프레임워크:**
- SWOT 분석
- 제품/가격 비교
- 마케팅 전략 분석

---

### 7. A/B 테스트
**폴더:** `external-skills/ab-test/`

**요청 예시:**
- "A/B 테스트 설계해줘"
- "전환율 테스트 계획 세워줘"

**기능:**
- 가설 수립
- 샘플 크기 계산
- 결과 분석

---

### 8. 마케팅 심리학
**폴더:** `external-skills/marketing-psychology/`

**요청 예시:**
- "구매 유도 심리 기법 알려줘"
- "사회적 증거 활용법 알려줘"

**기법 (70+개):**
- 희소성/긴급성 (Scarcity)
- 사회적 증거 (Social Proof)
- 앵커링 효과
- 손실 회피
- 호혜성의 원칙

---

### 9. 소셜 콘텐츠
**폴더:** `external-skills/social-content/`

**요청 예시:**
- "인스타그램 콘텐츠 캘린더 만들어줘"
- "릴스 아이디어 10개 줘"

**플랫폼:**
- Instagram (피드, 스토리, 릴스)
- YouTube (숏츠, 롱폼)
- 네이버 블로그

---

### 10. 가격 전략
**폴더:** `external-skills/pricing-strategy/`

**요청 예시:**
- "가격 전략 수립해줘"
- "번들 패키지 설계해줘"

**프레임워크:**
- 가치 기반 가격
- 심리적 가격
- 번들링/언번들링
- 프리미엄 포지셔닝

---

### 11. 출시 전략
**폴더:** `external-skills/launch-strategy/`

**요청 예시:**
- "신제품 출시 계획 세워줘"
- "런칭 캠페인 설계해줘"

**단계:**
- 사전 출시 (티저)
- D-Day 캠페인
- 사후 육성

---

### 12. 유료 광고
**폴더:** `external-skills/paid-ads/`

**요청 예시:**
- "메타 광고 전략 세워줘"
- "네이버 검색광고 설계해줘"

**플랫폼:**
- Meta (Facebook/Instagram)
- Google Ads
- 네이버 검색광고
- 카카오모먼트

---

### 13. 추천 프로그램
**폴더:** `external-skills/referral-program/`

**요청 예시:**
- "추천인 프로그램 설계해줘"
- "바이럴 루프 만들어줘"

**구성요소:**
- 보상 구조
- 참여 메커니즘
- 추적 시스템

---

### 14. 분석/추적 설정
**폴더:** `external-skills/analytics-tracking/`

**요청 예시:**
- "GA4 이벤트 설정해줘"
- "메타 픽셀 설치해줘"

**도구:**
- Google Analytics 4
- Meta Pixel
- 전환 추적
- UTM 파라미터

---

### 15. 영상 스크립트
**폴더:** `external-skills/video-script/`

**요청 예시:**
- "제품 소개 영상 스크립트 써줘"
- "고객 후기 영상 구성해줘"

**템플릿:**
- 제품 소개 (30초/60초)
- 고객 후기
- 튜토리얼
- 브랜드 스토리

---

### 16. 리뷰 관리
**폴더:** `external-skills/review-management/`

**요청 예시:**
- "리뷰 수집 전략 세워줘"
- "부정 리뷰 대응 가이드 만들어줘"

**기능:**
- 리뷰 수집 자동화
- 긍정/부정 리뷰 대응
- 리뷰 인센티브 설계

---

### 17. 마케팅 아이디어
**폴더:** `external-skills/marketing-ideas/`

**요청 예시:**
- "마케팅 아이디어 10개 줘"
- "저비용 고효율 마케팅 방법 알려줘"

**카테고리 (140개 아이디어):**
- 콘텐츠 마케팅
- 커뮤니티 빌딩
- 파트너십
- 오프라인 연계
